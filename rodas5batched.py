...
class Rodas5Batched(AbstractAdaptiveSolver):
    ...
    def step(
        ...
        # jac_f_batched = jax.jacobian(lambda _y: terms.vf(t=t0, y=_y, args=args))
        jac_f_batched = jax.vmap(lambda _t, _y, _a: jax.jacobian(lambda __y: f(_t, __y, _a))(_y),
                                (None, 0, 0)) 
        #J = jax.jacfwd(lambda y: terms.vf(t=t0, y=y, args=args))(y0)


        lu_batched = jax.vmap(lambda a: jax.scipy.linalg.lu_factor(I/dtgamma - a), 
                              (0,))
        ...
        # dt_f_batched = jax.jacobian(lambda _t: terms.vf(t=_t, y=y0, args=args))
